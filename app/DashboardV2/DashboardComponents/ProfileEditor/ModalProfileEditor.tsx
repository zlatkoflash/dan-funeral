...
export default function ModalProfileEditor() {
  ...
  const showModal = useAppSelector(
    (state) => state.dashboard.modalShow_ProfileDetails,
  );
  const activeTab = showModal.type;

  const navigationMap: {
    key: "my-profile" | "bussiness" | "password" | "membership";
    label: string;
    path: string;
  }[] = [
    { key: "my-profile", label: "My Profile", path: "/DashboardV2" },
    {
      ...
      path: "/DashboardV2/EditBusiness/LocationsAndCategories",
    },
    {
      key: "password",
      label: "Password",
      path: "/DashboardV2/EditBusiness/MediaGallery",
    },
    {
      key: "membership",
      label: "Membership",
      path: "/DashboardV2/EditBusiness/ServicesAndPrices",
    },
  ];
    ...
    <>
      <Modal
        ...
        onHide={() => {
          ...
          dispatch(
            dashboardSlice.actions.setModalShow_ProfileDetails({
              show: false,
              type: showModal.type,
            }),
          );
          ...
          <div className="business-editor-wrap">
            <div className="business-editor-content">
              <Nav
                variant="tabs"
                activeKey={activeTab}
                className="border-bottom-0 tabs-profile-editor"
              >
            ...
            <div className="profile-tabs-content">
              ...
              {activeTab === "membership" && <PlansVersion2 plansOut={plans} />}
            </div>
          </div>
        </Modal.Body>
      </Modal>
      ...
      <form onSubmit={() => {}} className="form-dashboard">
        <Container>
          ...
          {successMessage !== "" && (
            <Row>
              <Col>
                <div className="text-let text-success">{successMessage}</div>
              </Col>
            </Row>
          )}
          {errorMessage !== "" && (
            <Row>
              <Col>
                <div className="text-let text-danger">{errorMessage}</div>
              </Col>
            </Row>
          )}
        </Container>
...
function BusinessProfileEditor() {
  ...
  const [official_business_name, setOfficial_business_name] = useState(
    user?.business_profile?.official_business_name ?? "",
  );
  const [business_url, setBusiness_url] = useState(
    user?.business_profile?.business_url ?? "",
  );
  const [business_email, setBusiness_email] = useState(
    user?.business_profile?.business_email ?? "",
  );
  const [business_phone, setBusiness_phone] = useState(
    user?.business_profile?.business_phone_number ?? "",
  );
  const [business_location, setBusiness_location] = useState(
    user?.business_profile?.business_location ?? "",
  );
      ...
      <form onSubmit={() => {}} className="form-dashboard">
        <Container>
          <Row>
            <Col md={6}>
              <TextInput
                ...
                label=""
                onChange={(e) => {
                  setOfficial_business_name(e.target.value);
            ...
            <Col md={6}>
              <TextInput
                ...
                label=""
                onChange={(e) => {
                  setBusiness_url(e.target.value);
                }} // TODO: Implement state management
                type="url"
                value={business_url}
          ...
          <Row>
            ...
            <Col md={6}>
              <TextInput
                ...
                label=""
                onChange={(e) => {
                  setBusiness_phone(e.target.value);
                }} // TODO: Implement state management
                type="tel"
          ...
          <Row>
            <Col md={6}>
              <TextInput
                ...
                label=""
                onChange={(e) => {
                  setBusiness_location(e.target.value);
                }} // TODO: Implement state management
                type="text"
          ...
          {message !== "" && (
            <Row>
              <Col>
                <div className="text-let text-success">{message}</div>
              </Col>
            </Row>
          )}
          {errorMessage !== "" && (
      ...
      >
        <Container>
          ...
          <Row className="row-buttons">
            <Col>
              <Link
                href={"/Dashboard/User/Auth/ForgotPassword"}
