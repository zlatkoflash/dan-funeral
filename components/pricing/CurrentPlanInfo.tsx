...
export default function CurrentPlanInfo() {
  ...
  const { user } = useAuth();
  const [plan, set_plan] = useState<StripeProductWithPrices | null | undefined>(null);

  const loadStripePlans = async () => {
    const res = await getStripePlans();
    console.log("Stripe plans:", res);

    const planFound = res.find((plan) => plan.metadata.plan_type === user?.defaultListing.planType);
    console.log("planFound 2:", planFound);
    set_plan(planFound);
  ...
  return <>
    <div className="pricing-grid-wrapper">
      <div
        className="pricing-grid current-plan"
      >
