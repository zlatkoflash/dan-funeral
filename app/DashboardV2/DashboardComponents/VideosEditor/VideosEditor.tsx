...
export default function VideosEditor(
  ...
  const ___DeleteTheVideo = (video: IVideoDetails) => {
    ...
    setVideos((prevVideos) => prevVideos.filter((v) => v.mp4_file.filename !== video.mp4_file.filename));
  }

  const __LOadTheVideos = async () => {


    const result = await getApiData<{ ok: boolean, media_gallery_videos: IVideoDetails[] }>("/listings/GetVideosDetails", "POST", { listing_id: user.defaultListing.id }, "authorize");
  ...
  return <>
    <div className="media-editor text-input-wrap">

      {
        ...
        <>
          ...
          <div className="media-editor-grid">
            ...
            {
              (user.defaultListing.planType === "standard" && videos.length >= user.defaultListing.counts.videos) && <div className="media-editor-grid-item add-photo-item" onClick={() => {
                dispatch(
      ...
      {
        user?.defaultListing.planType === "basic" && <div className="video-editor-locked">
          <div className="icon">
