...
export default function ModalUserAuthSingUpV2(data: IModalUserAuthSingUp) {
  const [stepsStatuses, set_stepsStatuses] = useState<
    { title: string; status: "active" | "completed" | "pending" }[]
  >([
    { title: "Account", status: "active" },
    { title: "Your Business", status: "pending" },
  ]);
  const [account_steps, set_account_steps] = useState<
    "signup-form" | "select-your-business" | "all-set"
  >("signup-form");

  return (
    <>
      <AccountProgress steps={stepsStatuses} />

      {account_steps === "signup-form" && (
        <SignupForm
      ...
      >
        <Container>
          ...
          <Row className="row-already-login-footer">
            <Col>No credit card required</Col>
          </Row>
          <Row className="row-already-login-footer">
            <Col>
...
function SelectYourBusinessForm() {
  ...
  const [listing_id_for_claiming, set_listing_id_for_claiming] =
    useState<string>("");
  ...
  const FindListingsAttachedForLoggedUser = async (search: string) => {
    const resultsAfterFindingListings = await getApiData<{
      ...
      "/listings/FindListingsAttachedForLoggedUser",
      "POST",
      {
  ...
  const ClaimTheBussiness = async () => {
    ...
    const resultsAfterClaimingTheBusiness = await getApiData(
      "/listings/ClaimTheBusiness",
      "POST",
      {
    ...
    console.log(
      "resultsAfterClaimingTheBusiness",
      resultsAfterClaimingTheBusiness,
    );
        ...
        <Container>
          <Row>
            <Col className="text-center">
              ...
              <p>
                Search to claim an existing listing, or create a new profile
                from scratch.
              </p>
            </Col>
          </Row>
          ...
          <Row className="justify-content-space-between">
            <Col lg={6}>Business name or Address</Col>
