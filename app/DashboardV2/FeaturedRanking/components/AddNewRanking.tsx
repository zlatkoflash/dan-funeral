...
export default function AddNewRanking({
  ...
  const [toggleTarget, setToggleTarget] = useState<
    "rank-by-service" | "rank-by-location"
  >("rank-by-service");
  ...
  const selectedCategoryObject = useMemo(() => {
    return categories.find(
      (category) => category.term_id.toString() === selectedCategoryId,
    );
  }, [categories, selectedCategoryId]);
  const selectedSubcategoryObject = useMemo(() => {
    return selectedCategoryObject?.children.find(
      (category) => category.term_id.toString() === selectedSubServiceId,
    );
  }, [selectedCategoryObject, selectedSubServiceId]);

  const [locationId, setLocationId] = useState<string>("");
  const locationObject = useMemo(() => {
    return user.defaultListing.data.services_areas_and_categories.locations.find(
  ...
  const [stripeRankingProducts, setStripeRankingProducts] = useState<
    StripeProductWithPrices[]
  >([]);
  ...
  const [rankingProduct, setRankingProduct] =
    useState<StripeProductWithPrices | null>(null);
  ...
  const AddUpdateItemNewRankingToCard = async () => {
    ...
    const locationObject =
      user.defaultListing.data.services_areas_and_categories.locations.find(
        (location) => location.place_id === locationId,
      );
    ...
    let objectRanking: IRankData = {
      ...
      rank_position:
        rankingProduct?.metadata?.rank_type !== undefined
          ? Number(rankingProduct?.metadata?.rank_type)
          : 3,
    };
    ...
    if (toggleTarget === "rank-by-service") {
      ...
      if (catObj?.children.length === 0) {
        ...
        serviceObject = catObj?.children.find(
          (category) => category.term_id.toString() === selectedSubServiceId,
        );
      }
      objectRanking = {
    ...
    if (initialData === undefined) {
      const resultSavingItem = await getApiData<{
        ...
        "/listings/RankingCard_AddRankingItem",
        "POST",
        {
      ...
      dispatch(
        shopSlice.actions.addRankItemToCard({
          // here go updated item
          rankData: resultSavingItem.item,
          product: stripeRankingProducts.find(
            (product) => product.id === rankingProductId,
      ...
      const resultSavingItem = await getApiData<{
        ...
        "/listings/RankingCard_UpdateRankingItem",
        "POST",
        {
          ranking_item: objectRanking,
        },
      ...
      dispatch(
        shopSlice.actions.updateRankItemInCard({
          // here go updated item
  ...
  const findCategoryDeep = (
    categoriesFor: ILE10ServiceOffering[],
    targetId: number,
  ): {
  ...
  const SetInitialDataIfIsEditor = async () => {
    ...
    setToggleTarget(
      initialData.type === "category" ? "rank-by-service" : "rank-by-location",
    );
    setRankingProductId(initialData.stripe_product_id as string);
    setRankingProduct(
      stripeRankingProducts.find(
        (p) => p.id === initialData.stripe_product_id,
      ) as StripeProductWithPrices,
    );

    console.log("initialData:", initialData);

    const serviceData = findCategoryDeep(
      categories,
      Number(initialData.service_id),
    );
                ...
                <div className="left-content">
                  <h5>
                    "
                    {
                      ...
                      (() => {
                        if (toggleTarget === "rank-by-service") {
                          if (selectedSubcategoryObject !== undefined)
                            return selectedSubcategoryObject.name;
                          if (selectedCategoryObject !== undefined)
                            return selectedCategoryObject.name;
                        }
                        if (toggleTarget === "rank-by-location") {
                          if (locationId !== "") {
                            return (
                    ...
                    {(() => {
                      if (rankingProduct !== null) {
                        const rankPosition = Number(
                          rankingProduct?.metadata?.rank_type,
                        );
                        if (rankPosition === 1) {
                          return <>1st AD</>;
                        }
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        const feedbackAfterRemovingData = await getApiData<{
                          ...
                          "/listings/RankingCard_RemoveItemFromCart",
                          "POST",
                          {
                            // listing_id: user.defaultListing.id,
                        ...
                        console.log(
                          "feedbackAfterRemovingData:",
                          feedbackAfterRemovingData,
                        );
                        if (feedbackAfterRemovingData.ok) {
                          dispatch(
                            shopSlice.actions.RemoveCardItemFromCard(
                              initialData.id || 0,
                            ),
                          );
        ...
        <form className="form-dashboard">
          <Container>
                    ...
                    <Row>
                      <Col>
                        <TagsButtonsPlusMinus
                          ...
                          tags={[
                            ...(selectedCategoryObject?.children?.map(
                              (child) => {
                                return {
                  ...
                  <Col>
                    <TextInput
                      ...
                      options={[
                        ...
                        ...user.defaultListing.data.services_areas_and_categories.locations.map(
                          (location) => {
                            return {
                  ...
                  <div className="ranking-position-radios">
                    {stripeRankingProducts.map((rankProduct, index) => {
                      return (
                        <label
                          key={index}
                          htmlFor={`ranking-position-${index}`}
                          className="ranking-position-radio"
                        >
                          <input
                            type="radio"
                            name="ranking-position"
                            id={`ranking-position-${index}`}
                            checked={rankingProductId === rankProduct?.id}
                            onChange={() => {
                          ...
                          <div className="the-content">
                            ...
                            {formatPrice(
                              (rankProduct.monthly?.unit_amount || 0) / 100,
                            )}
                            /month
                          </div>
                          ...
                          <div className="dot-icon">
                            <img
                              src={dot_solid_circle.src}
                              alt="icon"
                              className="checked-dot"
                            />
                            <img
                              src={dot_not_solid.src}
                              alt="icon"
                              className="not-checked-dot"
                            />
                          </div>
                        </label>
                      );
            ...
            <Row className="row-buttons">
              <Col>
                <Link
                  href="/DashboardV2/FeaturedRanking/"
                  className={`btn btn-light ${loading ? "loading" : ""}`}
                  onClick={() => {
                    // ___SaveThePart();
                  }}
                >
