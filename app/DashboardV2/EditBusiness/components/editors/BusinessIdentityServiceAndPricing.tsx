...
import ServicesEditor, { IOtherService } from "@/app/DashboardV2/DashboardComponents/ServicesEditor/ServicesEditor";
...
export default function BusinessIdentityServiceAndPricing() {
  ...
  const fetchServices = async () => {
    ...
    }>("/listings/GET_OtherServices", "POST", {
  ...
  const updateServices = async (services: IOtherService[]) => {
    const resultServices = await getApiData<{
      ok: boolean,
      message: string
    }>("/listings/UPDATE_OtherServices", "POST", {
      listing_id: user.defaultListing.id,
      services: services
    }, "authorize", "application/json");
    console.log("resultServices:", resultServices);
    ...
    <>
      <div className="panel-content-wrap">
        ...
        <div className="heading">
          ...
          <p>These tags are unlimited across all plans. They power internal search and improve SEO.</p>
        </div>
        ...
        <form onSubmit={() => { }} className="form-dashboard">
          <Container>
            ...
            <Row className="row-buttons">
              <Col>
                <Button variant="light" type="button" className={loading ? "loading" : ""} disabled={loading} onClick={() => { SaveServices(false); }}>
                  Save The Draft
                </Button>

                <Button variant="success" type="button" className={loading ? "loading" : ""} disabled={loading} onClick={() => { SaveServices(true); }}>
                  Save & Continue
                </Button>
              </Col>
            </Row>
